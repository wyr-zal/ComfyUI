...
| --- | --- | --- |
| 已提交源码 | `custom_nodes/company_remote/`、相关测试和 `server.py` 配套改动，基线为 `d509e130`。 | 已跟踪的文件可以得到；被忽略目录中的新增文件仍需单独确认。 |
| 当前工作树扩展 | 当前工作区在 `d509e130` 之后仍有未提交的 `long_video.py`、`nodes.py` 和 `test_long_video.py` 真实功能差异。 | **不会**随 checkout `d509e130` 得到，必须在交付前提交、导出 patch 或单独打包。 |
| 用户运行数据 | 工作流 JSON、输入视频、配置、资产库、manifest、断点和输出视频。 | 大部分位于 `.gitignore` 忽略目录，必须作为独立交付物或按需迁移。 |
...
截至本文日期，以下文件相对于 `d509e130` 存在真实代码差异，不是单纯 CRLF/LF 噪声：
...
如果本次交付以当前工作区为准，必须把上述源码和测试一起交付，并在交接单中记录实际 commit 或 patch 校验值。如果只交付 `d509e130`，接手方只能得到已提交基线，不能使用这些未提交扩展。
...
2. 已提交版本 `d509e130` 对应的 Company Remote 源码，以及 `server.py` 的配套差异。
...
这些文件当前位于 `user/default/workflows/`；该目录默认被 `.gitignore` 忽略，必须显式作为工作流包交付：

| 文件 | 用途 |
| --- | --- |
| `视频欧美转绘_长视频_分镜检测测试.json` | 只做本地镜头检测和预览，不调用视频模型。 |
| `视频欧美转绘_长视频_连续分镜生成测试.json` | 选择连续 1–3 个镜头做低成本端到端测试。 |
| `人物视频多风格转绘_长视频_Seedance版_v3手动批次_1分钟审阅_流水线.json` | 推荐的 v3 手动批次生产入口，按批次审阅后继续。 |
| `人物视频多风格转绘_长视频_Seedance版_v3手动批次_1分钟审阅_含入库素材库查看.json` | 在批次旁查看当前和历史入库素材，适合审阅。 |
| `人物视频多风格转绘_长视频_按镜头自动资产转绘_Seedance版_素材库复用v3_短镜头合并_音频可选_限时生成.json` | 自动规划、短镜头合并和音频开关的 v3 长视频入口。 |
| `Wan2.7_三人物参考图直传_无TOS无火山_20秒完整处理.json` | 三人物 Wan2.7 显式三段处理样例。 |
| `三人物ABC_火山资源ID_Seedance转换_10秒验证.json` | 资产 ID 注册与 Seedance 小样验证。 |

当前工作树另有 `人物视频多风格转绘_长视频_Seedance版_v3手动批次_1分钟审阅_仅人物转绘保留背景.json` 等依赖未提交 `person_only` 节点的工作流。若交付该工作流，必须同时交付对应未提交源码和测试；不能把它与 `d509e130` 单独打包。
...
6. 如果将当前工作树扩展纳入交付，先锁定其 commit/patch，再同步工作流 JSON；不要交付“源码是旧版、工作流是新版”的混合包。
...
- [ ] 已明确交付的是 `d509e130` 基线，还是包含当前未提交扩展的工作树版本。
- [ ] 已单独导出被 `.gitignore` 忽略的推荐工作流，并记录文件校验值。
